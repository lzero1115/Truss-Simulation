...
class TrussOptimizerNonconvex:
    ...
    def _assemble_global_stiffness(self, design_rho):
        ...
        for idx, i in enumerate(self.design_edge_indices):
            K_global += design_rho[idx] * self.K_diff_array[i]
        return K_global
    ...
    # ∂(u^T)/∂(rho_j) = -u^T * K_j * K^-1,  since K and K_i, K_j symmetric
    # -∂(u^T * K_i * u)/∂(rho_j) = -∂(u^T)/∂(rho_j) * K_i * u - u^T * K_i * ∂(u)/∂(rho_j)
    # -∂(u^T)/∂(rho_j) * K_i * u - u^T * K_i * ∂(u)/∂(rho_j)
    # = u^T * K_j * K^-1 * K_i * u + u^T * K_i * K^-1 * K_j * u
    # = u^T * (K_j * K^-1 * K_i + K_i * K^-1 * K_j) * u
    ...
    def solve_optimization(self):
        """Solve the nonconvex compliance minimization problem using Knitro."""
        print(f"Starting nonconvex optimization with Knitro...")
        ...
        KN_set_int_param(kc, KN_PARAM_DERIVCHECK, 3)
        KN_set_int_param(kc, KN_PARAM_OUTLEV, KN_OUTLEV_ITER)
        KN_set_int_param(kc, KN_PARAM_OUTMODE, KN_OUTMODE_SCREEN)
        KN_set_int_param(kc, KN_PARAM_ALGORITHM, KN_ALG_BAR_DIRECT)  # Interior point direct method
        KN_set_int_param(kc, KN_PARAM_BAR_MURULE, KN_BAR_MURULE_ADAPTIVE)  # Adaptive barrier updates
        KN_set_int_param(kc, KN_PARAM_HESSIAN_NO_F, KN_HESSIAN_NO_F_ALLOW)
        KN_set_double_param(kc, KN_PARAM_FEASTOL, 1e-6)  # Match SDP tolerance
        KN_set_double_param(kc, KN_PARAM_OPTTOL, 1e-6)  # Match SDP tolerance
        KN_set_int_param(kc, KN_PARAM_LINSOLVER, KN_LINSOLVER_MA57)  # Efficient sparse linear solver
        KN_set_int_param(kc, KN_PARAM_MAXIT, 300)  # Increase max iterations

        # Add variables (design densities) - ONLY for design edges
        KN_add_vars(kc, self.n_design_vars)

        # Set variable bounds (1e-5 lower bound to avoid singularity)
        KN_set_var_lobnds(kc, xLoBnds=[1e-6] * self.n_design_vars)
        KN_set_var_upbnds(kc, xUpBnds=[1.0] * self.n_design_vars)

        # # Add volume constraint as a LINEAR constraint
        KN_add_cons(kc, 1)
        # KN_set_con_eqbnds(kc, cEqBnds=[self.target_volume])  # volume = target_volume
        # Set a one-sided inequality constraint: volume ≤ target_volume
        KN_set_con_upbnds(kc, cUpBnds=[self.target_volume])
        # Optionally set lower bound as 0 if you want to be explicit (though this is typically the default)
        KN_set_con_lobnds(kc, cLoBnds=[0.0])

        # Set the linear coefficients for the volume constraint
        # c^T x = target_volume, where c is the vector of volume coefficients
        ...
        # Initialize to intermediate density that satisfies volume constraint
        initial_rho = np.ones(self.n_design_vars) * (self.target_volume / np.sum(self.design_volumes))
        initial_rho = np.clip(initial_rho, 1e-6, 1.0)  # Ensure bounds are respected

        # Initial point
        KN_set_var_primal_init_values(kc, xInitVals=initial_rho)
...
class TrussVisualizer:
    ...
    def callback(self):
        ...
        if psim.Button("Run Nonconvex Optimization"):
            optimizer = TrussOptimizerNonconvex(self.truss_optimized, self.get_force_vector(),
                                                volume_fraction=self.volume_fraction)
            self.optimized_densities = optimizer.solve_optimization()
            if self.optimized_densities is not None:
