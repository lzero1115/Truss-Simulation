...
class TrussOptimizer:
    ...
    def solve_optimization(self):
        ...
        # Diagnostics: Check the PSD property of K_fixed before any adjustments
        min_eig_before = np.linalg.eigvalsh(K_fixed).min()
        print(f"Initial minimal eigenvalue of K_fixed: {min_eig_before:.3e}")

        # Scale, regularize, and symmetrize K_fixed
        # (In this relaxed version, we assume no scaling is applied here.
        #  Adjust scaling as needed for your application.)
        K_fixed *= 1.0  # If you have a scaling factor, apply it here.
        K_fixed += epsilon * np.eye(n_dofs)
        K_fixed = 0.5 * (K_fixed + K_fixed.T)
        # Check minimal eigenvalue after regularization and symmetrization
        min_eig_after = np.linalg.eigvalsh(K_fixed).min()
        print(f"Minimal eigenvalue of K_fixed after regularization: {min_eig_after:.3e}")

        # Enforce a minimum eigenvalue threshold (desired minimum eigenvalue)
        desired_min_eig = 1e-8
        if min_eig_after < desired_min_eig:
            extra_reg = (desired_min_eig - min_eig_after)
            K_fixed += extra_reg * np.eye(n_dofs)
            print(f"Added extra diagonal shift: {extra_reg:.3e}")
        else:
            print("No extra diagonal shift needed.")

        # Final diagnostic before passing to the SDP solver
        final_min_eig = np.linalg.eigvalsh(K_fixed).min()
        print(f"Final minimal eigenvalue of K_fixed (before SDP solve): {final_min_eig:.3e}")
